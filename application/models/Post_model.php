...
class Post_model extends CI_Model
{
	...
	//get today posts
	public function get_today_posts($limit)
	{

		// $minvalue = new DateTime("now");
		
		$date = new DateTime("now");
 		$minvalue = date('Y-m-d H:i:s',strtotime('yesterday'));
		$maxvalue = '2050-12-31 23:59:59';
		$this->set_filter_query();
		$this->db->where("posts.created_at BETWEEN '$minvalue' AND '$maxvalue'");
		$this->db->order_by('id', 'DESC');
		$this->db->limit($limit);
		$query = $this->db->get('posts');
		return $query->result();
